...
const VELOCITY_CAP = 59;
const UNVERIFIED_CAP = 79;
...
export async function scoreContract(
  ...
  let raw =
    verificationPts + agePts + txPts + interactorPts + deployerPts + arcPts;
  if (raw < 0) raw = 0;
  if (raw > 100) raw = 100;
  ...
  console.log(
    `[contract-score] ${address} verify=${verificationPts} age=${agePts} ` +
      ...
      `arc=${arcPts} velocity=${velocityFlag} ` +
      `raw=${raw} cap=${cap} final=${score}`
  );
